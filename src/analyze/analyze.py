...
class AggregateCheckpoints:
    """A class for loading and processing model checkpoints for deep ensemble (DE)
    and deep evidential regression (DER) models.
    ...
    Methods:
        load_checkpoint(model_name, prescription, inject_type, data_dim, noise, epoch, device, path="models/",
                        BETA=0.5, nmodel=1, COEFF=0.5, loss="SDER", load_rs_chk=False, rs=42,
        ...
        ep_al_checkpoint_DER(checkpoint):
            Extracts epistemic and aleatoric uncertainty metrics (mean and standard
            deviation) from a loaded DER model checkpoint.
    """
    ...
    def load_checkpoint(
        ...
        """Loads a PyTorch model checkpoint from a .pt file, constructing the file name based on model type (DE or DER) and other configuration parameters.
        ...
        Parameters:
            ...
            path (str): Directory path to the model checkpoints (default: "models/").
            ...
            load_rs_chk (bool): Flag to indicate if random seed checkpoint is used (default: False).
            rs (int): Random seed value if load_rs_chk is True (default: 42).
            load_nh_chk (bool): Flag to indicate if a specific hidden layer configuration is used (default: False).
            nh (int): Number of hidden units if load_nh_chk is True (default: 64).

        Returns:
            dict: The loaded checkpoint containing model weights and additional data.
        """
        print(model_name)
    ...
    def ep_al_checkpoint_DER(self, checkpoint):
        """Extracts mean and variance validation metrics from a loaded DER model
        checkpoint.
